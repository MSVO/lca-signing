# FDM CA Cryptosystem Simulator. Written by Adam Clarridge
# (3agc AT queensu D0T ca), 2008
...
class FDM_CA:
    ...
    def __init__(self, dimension, state_set, neighbourhood):
        ...
        #the neighbourhood is a tuple of tuples representing
        # the set of dimension-
        #sized vectors corresponding to the CA's
        #neighbourhood.
        #Example: neighbourhood = ((1,0),(0,1),(-1,0),
        #(0,-1)) is the 2-dimensional von Neumann
        # neighbourhood without the cell itself
        self.neighbourhood = neighbourhood

        #the transition_rules dictionary works as follows:
        #if the neighbourhood of a cell c is part of this
        #CA's acting_set, then if the state of c is s, c
        #will change to transition_rules[s] on the next
        #generation.
        ...
        #the acting set is a list of tuples corresponding to
        # the neighbourhood positions in the order of the
        #self.neighbourhood tuple.
        #Example: acting_set = [(s1,s2,s3,s4),(s1,s1,s1,s2)
        #,(s3,s1,s1,s2)] (here (s1,s2,s3,s4) corresponds to
        #               s2
        #            s3 c  s1
        #               s4
    ...
    #the "message" input is a dictionary keyed by cell
    #location. So a message encoded into a 2x2 array may
    #have the following entries:
    # message[(0,0)] = s1
    # message[(1,0)] = s2
    # message[(0,1)] = s3
    # message[(1,1)] = s4
    #messages in any rectangular shape are acceptable
    #(higher dimensions than 2 are OK as well)
    def apply(self, message, sizes):
        tmp={}
        for key in message.keys():
            nbhood = []
            for nb in self.neighbourhood:
                nbhood.append(message[vector_add\
                (key,nb,sizes)])
            if tuple(nbhood) in self.acting_set:
                tmp[key] = self.transition_rules\
                [message[key]]
            else:
                tmp[key] = message[key]
        return tmp
    ...
    def calculate_inverse(self):
        for state in self.state_set:
            self.inverse_transition_rules\
            [self.transition_rules[state]]=state
    ...
    def apply_inverse(self, message, sizes):
        ...
        for key in message.keys():
            ...
            for nb in self.neighbourhood:
                ...
                (key,nb,sizes)])
            if tuple(nbhood) in self.acting_set:
                tmp[key] = self.inverse_transition_rules\
                [message[key]]
            else:
                tmp[key] = message[key]
        return tmp

#Creates a sequence of FDM RCA at initialization and
#composes them together to form one CA. Also provides
...
class FDM_CA_Cryptosystem():
    ...
    def write_key(self, filename, key_type):
        if key_type=="public":
            ...
            for nb in range(0,len(self.neighbourhood)):
                for num in range(len(self.neighbourhood\
                [nb])):
                    nbhd_str=nbhd_str + \
                    str(self.neighbourhood[nb][num])
            ...
            for nbhd in self.composed_CA:
                ...
                for state in self.composed_CA[nbhd]:
                    ...
                    rule2.setAttribute("next_state",\
                    str(self.composed_CA[nbhd][state]))
            ...
            privkey.setAttribute("num_FDM_CA", \
            str(len(self.FDM_CA_list)))
            privkey.setAttribute("dimension", \
            str(self.FDM_CA_list[0].dimension))
            ...
            for i in range(0,len(self.FDM_CA_list)):
                ...
                for state in ca.transition_rules:
                    ...
                    map_elem.setAttribute("state",\
                    str(state))
                    map_elem.setAttribute("next_state",\
                    str(ca.transition_rules[state]))
            ...
            print "Invalid key type. Please enter "+\
            "'public' or 'private'."
    ...
    def write_parameters(self,filename):
        ...
        for nb in range(0,len(self.neighbourhood)):
            for num in range(len(self.neighbourhood[nb])):
                nbhd_str=nbhd_str + str(self.neighbourhood\
                [nb][num])
                if num<len(self.neighbourhood[nb])-1:
                    nbhd_str=nbhd_str + ","
            if nb<len(self.neighbourhood)-1:
    ...
    def read_parameters(self, filename):
        try:
            ...
            print "Error: File either cannot be read or "+\
            "is malformed XML."
            go = False
        if go:
            params = dom.firstChild
            CA_params = params.getElementsByTagName\
            ("CA_parameters").item(0)
            alg_params = params.getElementsByTagName\
            ("algorithm_parameters").item(0)
            self.state_set = list(str(CA_params.\
            getAttribute("state_set")))
            self.dimension = int(str(CA_params.getAttribute\
            ("dimension")))
            tmp_nbhd = str(CA_params.getAttribute\
            ("neighbourhood")).split(";")
            self.neighbourhood = []
            for nb in tmp_nbhd:
                n = []
            ...
            self.num_CA = int(str(alg_params.getAttribute\
            ("numCA")))
            self.p = float(str(alg_params.\
            getAttribute("p")))
            self.q = float(str(alg_params.\
            getAttribute("q")))
    ...
    def read_key(self,filename,key_type):
        if key_type=="public":
            ...
            self.state_set = list(str(pubkey.getAttribute\
            ("state_set")))
            self.dimension = int(str(pubkey.getAttribute\
            ("dimension")))
            tmp_nbhd = str(pubkey.getAttribute\
            ("neighbourhood")).split(";")
            self.neighbourhood = []
            for nb in tmp_nbhd:
                n = []
                ...
                self.neighbourhood.append(tuple(n))
            self.composed_CA={}
            for nbhd_node in pubkey.getElementsByTagName\
            ("neighbourhood"):
                nbhd = tuple(list(str(nbhd_node.\
                getAttribute("configuration"))))
                self.composed_CA[nbhd]={}
                for mapping_node in nbhd_node.\
                getElementsByTagName("map"):
                    self.composed_CA[nbhd][mapping_node.\
                    getAttribute("state")]=mapping_node.\
                    getAttribute("next_state")
            ...
            state_set = list(str(privkey.getAttribute\
            ("state_set")))
            tmp_nbhd = str(privkey.getAttribute\
            ("neighbourhood")).split(";")
            neighbourhood = []
            for nb in tmp_nbhd:
                n = []
            ...
            num_FDM_CA = int(str(privkey.getAttribute\
            ("num_FDM_CA")))
            self.dimension = int(str(privkey.getAttribute\
            ("dimension")))
            self.FDM_CA_list = []
            for i in range(num_FDM_CA):
                self.FDM_CA_list.append(FDM_CA(\
                self.dimension,state_set,neighbourhood))

            for ca_node in privkey.getElementsByTagName\
            ("FDM_CA"):
                num = int(str(ca_node.getAttribute\
                ("number")))
                for acting_set_node in ca_node.\
                getElementsByTagName("acting_set"):
                    for nbhd_node in acting_set_node.\
                    getElementsByTagName("nbhd"):
                        nbhd_tup = tuple(list(str(nbhd_node\
                        .getAttribute("conf"))))
                        self.FDM_CA_list[num].acting_set.\
                        append(nbhd_tup)
                for function_node in ca_node.\
                getElementsByTagName("function"):
                    for map_node in function_node.\
                    getElementsByTagName("map"):
                        self.FDM_CA_list[num].\
                        transition_rules[str(map_node.\
                        getAttribute("state"))]=str(\
                        map_node.getAttribute("next_state"))
    ...
    def generate_keys(self,dimension,state_set,\
    neighbourhood,num_CA,p,q):
        self.dimension = dimension
        self.state_set = state_set[:]
        self.neighbourhood = neighbourhood
        ...
        self.FDM_CA_list = []
        self.all_possible_neighbourhoods = self.\
        generate_all_possible_neighbourhoods()
        ...
        #Begin the algorithm to generate a random FDM CA
        #list
        change_set = []
        ...
        for i in range(0,self.num_CA):
            ca = FDM_CA(self.dimension,self.state_set,\
            self.neighbourhood)
            ...
            positions = self.random_binary(\
            self.neighbourhood_size)
            for nbhd in self.all_possible_neighbourhoods:
                unchanging_neighbourhood = True
                rg = range(self.neighbourhood_size)
                for n in rg:
                    if nbhd[n] in change_set:
                        unchanging_neighbourhood = False
                        if positions[n]==True:
                            #accept the neighbourhood for
                            #sure in this case
                            ca.acting_set.append(nbhd)
                            break
                #if none of the neighbourhood states are
                #part of the change set, then accept the
                #neighbourhood with probability q
                if unchanging_neighbourhood == True and \
                random.uniform(0.0,1.0)<q:
                    ca.acting_set.append(nbhd)

            ca.transition_rules = \
            self.random_transition_rules(change_set)
        ...
        self.compose_CA()
        self.generated = True
        print "Done. The public and private keys are "+\
        "now in memory."

    #generates a random binary string of a specified length
    #the binary string is useful in randomly choosing the
    #acting_set of an FDM_CA.
    ...
    #Generates a list of all possible neighbourhoods.
    #This is useful to iterate through when finding the
    #composed_CA.
    def generate_all_possible_neighbourhoods(self):
        ...
        for i in range(0,total):
            ...
            for j in range(self.neighbourhood_size):
                temp.append(self.state_set[num/(self.\
                num_states**(self.neighbourhood_size-j-1))])
                num-=(num/(self.num_states**(self.\
                neighbourhood_size-j-1)))*(self.\
                num_states**(self.neighbourhood_size-j-1))
            nbhds.append(tuple(temp))
        return nbhds

    #this function generates a random permutation of the
    #given change_set, and returns the result as a
    #dictionary of outputs keyed by inputs.
    ...
    #encrypt a message using each of the CA in succession.
    # This has the same result as encrypting with the
    # composed CA and was only used for debugging and
    ...
    #encrypt a message using the public key, the composed
    # CA.
    def encrypt_with_composed_CA(self,message,\
    num_iterations):
        sizes = [0]*self.dimension
        for key in message.keys():
            for i in range(0,self.dimension):
        ...
        for i in range(num_iterations):
            ...
            for key in message.keys():
                ...
                for nb in self.neighbourhood:
                    nbhood.append(message[vector_add(\
                    key,nb,sizes)])
                tmp[key] = self.composed_CA[tuple(nbhood)]\
                [message[key]]
            message = tmp
        return tmp

    #decrypts ciphertext by applying the inverse of each
    #rule in reverse order
    ...
    def compose_CA(self):
        ...
        all_possible_neighbourhoods:
            actors=[]
            #find out which rules will act on this
            #neighbourhood
            for c in range(0,len(self.FDM_CA_list)):
                if neighbourhood in self.FDM_CA_list[c].\
                acting_set:
                    actors.append(True)
                else:
                    actors.append(False)

            #simulate each rule acting on the neighbourhood
            #in succession,
            ...
            for s in self.state_set:
                ...
                for ca in range(0,len(self.FDM_CA_list)):
                    if actors[ca]:
                        state = self.FDM_CA_list[ca].\
                        transition_rules[state]
                self.composed_CA[neighbourhood][s] = state


#helps the user to create a 2D message with a specified
#number of rows and columns and the desired text, with
#random states filling up the rest of the message
...
def write_2D_ciphertext(ciphertext,filename,\
num_generations):
    print "Writing ciphertext to "+filename+"... ",
    rowsize,colsize=0,0
    for key in ciphertext.keys():
        i=key[0]
        j=key[1]
        if i>rowsize:
            rowsize=i
    ...
    ctext.setAttribute("num_generations",\
    str(num_generations))
...
def read_2D_message(filename):
    try:
        print "Reading message from "+filename+"... ",
        go=True
        dom = minidom.parse(filename)
    except:
        print "Error: File either cannot be read or "+\
...
def read_2D_ciphertext(filename):
    try:
        print "Reading ciphertext from "+filename+"... ",
        go=True
        dom = minidom.parse(filename)
    except:
        print "Error: File either cannot be read or is "+\
        "malformed XML."
        go = False
    if go:
        ...
        num_generations = int(ctext.getAttribute\
        ("num_generations"))
...
if __name__=="__main__":
    ...
    try:
        while True:
            ...
            print "    2. Read the algorithm parameters"+\
            " from a file"
            print "    3. Read a public key from a file"
            print "    4. Read a private key from a file"
            print "    5. Write the algorithm parameters"+\
            " to a file"
            ...
            if choice=='1':
                if sys.have_parameters == False:
                    print "You do not have the algorithm"+\
                    " parameters in memory. Please make"+\
                    " a selection:"
                    print "    1. Input parameters manually"
                    print "    2. Input parameters from a"+\
                    " file"

                    choice = raw_input("Please make a "+\
                    "selection.")
                    if choice == "1":
                        dim = int(str(raw_input("Please "+\
                        "enter the dimension size. "+\
                        "Currently the visual display of "+\
                        "non-2D messages is not "+\
                        "supported.")))
                        ss = list(str(raw_input("Please "+\
                        "enter the state set in a single "+\
                        "string, one state per character."+\
                        " Ex. 'abcd'.")))
                        nh = raw_input("Please enter the "+\
                        "neighbourhood of each cell "+\
                        "separated by semicolons, and do "+\
                        "not include spaces. Ex. "+\
                        "'0,0;1,0;-1,1' refers to the "+\
                        "cell itself, the right neighbour"+\
                        " and the top-left neighbour.")
                        tmp_nh = str(nh).split(";")
                        nh = []
                        for nb in tmp_nh:
                            n = []
                            for num in nb.split(","):
                                n.append(int(num))
                            nh.append(tuple(n))
                        nCA = int(str(raw_input("Please "+\
                        "enter the number of FDM CA you "+\
                        "would like in the composition.")))
                        prob1 = float(str(raw_input(\
                        "Please enter the probability p "+\
                        "(chance to add an element to the"+\
                        " 'change set'). Ex. '0.5'.")))
                        prob2 = float(str(raw_input(\
                        "Please enter the probability q "+\
                        "(chance to add a neighbourhood "+\
                        "containing no elements of the "+\
                        "change set to the acting set). "+\
                        "Ex. '0.5'.")))
                        sys.generate_keys(dim,ss,nh,nCA,\
                        prob1,prob2)
                        sys.have_parameters = True
                    elif choice=="2":
                        fn = str(raw_input("Please enter "+\
                        "the name of the file."))
                        #read parameters from a file
                        sys.read_parameters(fn)
                        sys.generate_keys(sys.dimension,\
                        sys.state_set,sys.neighbourhood,\
                        sys.num_CA,sys.p,sys.q)
                else:
                    sys.generate_keys(sys.dimension,sys.\
                    state_set,sys.neighbourhood,sys.num_CA,\
                    sys.p,sys.q)
            elif choice == '2':
                filename = raw_input\
                ...
                if sys.have_parameters:
                    ...
                    print "Error: No parameters are "+\
                    "loaded into memory."
            elif choice=='6':
                if sys.read_public==False and \
                sys.generated==False:
                    print "Generate or read a public key "+\
                ...
                if sys.read_private==False and \
                sys.generated==False:
                    print "Generate or read a private key"+\
                    " before trying to write."
                else:
                    filename = raw_input\
                    ("Please enter the filename.")
                ...
                if sys.generated or sys.read_public:
                    while True:
                        print "Please make a selection:"
                        print "    1. Manually input a"+\
                        " message"
                        print "    2. Read a message from"+\
                        " a file"
                        choice = raw_input("Please make a"+\
                        " selection.")
                        if choice == '1':
                            sys.rows = int(str(raw_input\
                            ("Please enter the number of "+\
                            "rows in your message.")))
                            sys.cols = int(str(raw_input\
                            ("Please enter the number of "+\
                            "columns in your message.")))
                            message = str(raw_input\
                            ("Please enter the message "+\
                            "you want to encrypt. "+\
                            "Remember to only use members"+\
                            " of the state set in your "+\
                            "message. If your message is "+\
                            "smaller than the specified "+\
                            "size, the rest of the "+\
                            "configuration will be set "+\
                            "randomly."))
                            msg = create_2D_message(list\
                            (message),sys.state_set,\
                            sys.rows,sys.cols)
                        elif choice == '2':
                            fn = raw_input\
                        ...
                        if choice =='1' or choice=='2':
                            sys.num_generations = int(str(\
                            raw_input("Please enter the "+\
                            ...
                            "Composed CA:"
                            encrypted_with_composed_CA = \
                            sys.encrypt_with_composed_CA\
                            (msg,sys.num_generations)
                            print_2D_message\
                            (encrypted_with_composed_CA)
                            choice = raw_input\
                            ("Would you like to write the"+\
                            " ciphertext to a file? (y/n)")
                            if choice=='y' or choice=='Y':
                                ...
                                write_2D_ciphertext(\
                                encrypted_with_composed_CA,\
                                fn,sys.num_generations)
                            choice = raw_input("Would you"+\
                            " like to write the original "+\
                        ...
                        print "Error: Can't encrypt, no"+\
                        " public key in memory."
            elif choice=='9':
                if sys.generated or sys.read_private:
                    fn = raw_input("Please enter the name"+\
                    ...
                    " write the decrypted message to a "+\
                    "file? (y/n)")
                    if choice=='y' or choice=='Y':
                        fn = raw_input("Please enter the "+\
                        "name of the file.")
                        write_2D_message(decrypted,fn)
                else:
                    print "Error: Can't decrypt, no "+\
