        ...
        while True:
            ...
            if choice=='1':
                if sys.have_parameters == False:
                    ...
                    choice = raw_input("Please make a selection.")
                    if choice == "1":
                        dim = int(str(raw_input("Please enter the dimension size. Currently the visual display of non-2D messages is not supported.")))
                        ss = list(str(raw_input("Please enter the state set in a single string, one state per character. Ex. 'abcd'.")))
                        nh = raw_input("Please enter the neighbourhood of each cell separated by semicolons, and do not include spaces. Ex. '0,0;1,0;-1,1' refers to the cell itself, the right neighbour and the top-left neighbour.")
                        tmp_nh = str(nh).split(";")
                        nh = []
                        for nb in tmp_nh:
                            n = []
                            for num in nb.split(","):
                                n.append(int(num))
                            nh.append(tuple(n))
                        nCA = int(str(raw_input("Please enter the number of FDM CA you would like in the composition.")))
                        prob1 = float(str(raw_input("Please enter the probability p (chance to add an element to the 'change set'). Ex. '0.5'.")))
                        prob2 = float(str(raw_input("Please enter the probability q (chance to add a neighbourhood containing no elements of the change set to the acting set). Ex. '0.5'.")))
                        sys.generate_keys(dim,ss,nh,nCA,prob1,prob2)
                        sys.have_parameters = True
                    elif choice=="2":
                        fn = str(raw_input("Please enter the name of the file."))
                ...
                if sys.generated or sys.read_public:
                    while True:
                        ...
                        choice = raw_input("Please make a selection.")
                        if choice == '1':
                            sys.rows = int(str(raw_input("Please enter the number of rows in your message.")))
                            sys.cols = int(str(raw_input("Please enter the number of columns in your message.")))
                            message = str(raw_input("Please enter the message you want to encrypt. Remember to only use members of the state set in your message. If your message is smaller than the specified size, the rest of the configuration will be set randomly."))
                            msg = create_2D_message(list(message),sys.state_set,sys.rows,sys.cols)
                        elif choice == '2':
                            fn = raw_input("Please enter the name of the file containing the message.")
                            msg,rows,cols = read_2D_message(fn)
                        if choice =='1' or choice=='2':
                            sys.num_generations = int(str(raw_input("Please enter the number of generations to evolve the CA.")))
                            ...
                            choice = raw_input("Would you like to write the ciphertext to a file? (y/n)")
                            if choice=='y' or choice=='Y':
                                fn = raw_input("Please enter the name of the file.")
                                write_2D_ciphertext(encrypted_with_composed_CA,fn,sys.num_generations)
                            choice = raw_input("Would you like to write the original message to a file? (y/n)")
                            if choice=='y' or choice=='Y':
                                fn = raw_input("Please enter the name of the file.")
                ...
                if sys.generated or sys.read_private:
                    fn = raw_input("Please enter the name of the file containing the ciphertext.")
                    ctext,gens = read_2D_ciphertext(fn)
                    print "Decrypted Message:"
                    decrypted = sys.decrypt(ctext,gens)
                    print_2D_message(decrypted)
                    choice = raw_input("Would you like to write the decrypted message to a file? (y/n)")
                    if choice=='y' or choice=='Y':
                        fn = raw_input("Please enter the name of the file.")
